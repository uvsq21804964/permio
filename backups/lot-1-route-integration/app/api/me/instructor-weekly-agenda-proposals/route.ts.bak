...
function getIsoDayOfWeekIndex(iso: string): number {
  // retourne 0 = lundi ... 6 = dimanche pour une date ISO "YYYY-MM-DD"
  const d = isoToDateOnly(iso);
  const jsDow = d.getDay(); // 0=dimanche, 1=lundi, ... 6=samedi
  const isoDow = jsDow === 0 ? 7 : jsDow; // 1..7 (lundi..dimanche)
  return isoDow - 1; // 0..6
}
...
async function buildClientSlotsForRange(params: {
  startDate: string; // début de la plage (inclus)
  endDate: string; // fin de la plage (inclus)
  ...
  while (cursor.getTime() <= end.getTime()) {
    ...
    const dayIndex = getIsoDayOfWeekIndex(dateIso); // 0 = lundi ... 6 = dimanche
  ...
  try {
    ...
    const bookedSlots = (await sql/* sql */ `
      ...
      WHERE "dogsitterUserId" = ${instructorId}
        AND "date" BETWEEN ${rangeStart}::date AND ${rangeEnd}::date
      ORDER BY "date", "startTime"
    `) as BookedSlot[];
    ...
    const clientSlotsByDate = await buildClientSlotsForRange({
