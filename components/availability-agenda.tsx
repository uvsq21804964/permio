...
export function AvailabilityAgenda() {
  ...
  const [timeRanges, setTimeRanges] = useState<TimeRange[]>([{ startTime: "08:00", endTime: "09:00" }])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < timeRanges.length; i++) {
      ...
      if (endMinutes <= startMinutes) {
        setError(`Plage ${i + 1}: L'heure de fin doit être après l'heure de début`)
        return
      }

      if (startMinutes < START_HOUR * 60 || endMinutes > 20 * 60) {
        setError(`Plage ${i + 1}: Les horaires doivent être entre 8h00 et 20h00`)
        return
      }
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (response.ok) {
        const refreshResponse = await fetch(`/api/availabilities?userId=${selectedUserId}`)
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-muted-foreground text-sm mt-2">
            Cliquez sur une case vide pour ajouter des disponibilités, ou sur une disponibilité existante pour la
            supprimer
          </p>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <div className="min-w-[800px]">
              ...
              <div className="grid grid-cols-8 gap-0">
                <div className="font-medium text-sm text-muted-foreground p-2 border-b">Heure</div>
                {DAYS.map((day) => (
                  <div key={day} className="font-medium text-sm text-center p-2 border-b border-r">
                    {day}
                  </div>
                ))}
              </div>
                ...
                {DAYS.map((_, dayIndex) => (
                  <div key={`day-${dayIndex}`} className="relative border-r">
                    ...
                    {getAvailabilitiesForDay(dayIndex).map((avail) => {
                      const { top, height } = getAvailabilityStyle(avail.startTime, avail.endTime)
                      const colorClass = getDurationColor(avail.startTime, avail.endTime)
                      return (
                        <div
                          key={avail.id}
        ...
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label>Jour</Label>
              <Input value={selectedDay !== null ? DAYS[selectedDay] : ""} disabled />
            </div>
