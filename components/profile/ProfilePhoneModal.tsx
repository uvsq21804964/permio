...
type ProfilePhoneModalProps = {
  draftPhoneCountryCode: string;
...
export function ProfilePhoneModal({
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-md rounded-2xl bg-card border shadow-lg p-5 space-y-4">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-sm font-semibold">{t('modals.phone.title')}</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <p className="text-xs text-muted-foreground">{t('modals.phone.helper')}</p>

        <form onSubmit={onSubmit} className="space-y-3">
          <div className="space-y-1.5">
            <label htmlFor="phone_country_code" className="text-xs font-medium text-foreground">
              {t('modals.phone.countryCode')}
            </label>
            <select
              id="phone_country_code"
              value={draftPhoneCountryCode}
              onChange={(event) => onChangeCountryCode(event.target.value)}
              className="w-full rounded-md border px-3 py-2 text-sm font-mono outline-none focus-visible:ring-2 focus-visible:ring-primary"
            >
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder={t('modals.phone.placeholder')}
              autoFocus
            />
          </div>
