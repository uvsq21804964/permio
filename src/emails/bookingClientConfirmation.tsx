// src/emails/bookingClientConfirmation.tsx
...
type BookingClientConfirmationEmailProps = {
...
export default function BookingClientConfirmationEmail({
  ...
  const previewText = fr
    ? `Votre créneau chez ${agencyName} a bien été réservé.`
    : `Your slot with ${agencyName} has been successfully booked.`;
  ...
  const title = fr ? 'Réservation confirmée' : 'Booking confirmed';

  const intro =
    instructorName?.trim() && serviceName?.trim()
      ? fr
        ? `Votre séance "${serviceName}" avec ${instructorName} est bien enregistrée.`
        : `Your "${serviceName}" session with ${instructorName} is confirmed.`
      : instructorName?.trim()
        ? fr
          ? `Votre séance avec ${instructorName} est bien enregistrée.`
          : `Your session with ${instructorName} is confirmed.`
        : serviceName?.trim()
          ? fr
            ? `Votre séance "${serviceName}" est bien enregistrée.`
  ...
  const ctaLabel = fr ? 'Voir mes réservations' : 'View my bookings';

  const noteText = fr
    ? `Vous pourrez retrouver ce rendez-vous à tout moment dans votre espace client.`
    : `You can find this appointment anytime in your client space.`;
                ...
                <tr>
                  ...
                  <td style={styles.topBarRight}>
                    <Text style={styles.topRightText}>
                      {fr ? 'Confirmation client' : 'Client confirmation'}
                    </Text>
                  </td>
                </tr>
              </tbody>
